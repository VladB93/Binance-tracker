...
import {
  ...
  REST_API_EXCHANGE_INFO_ENDPOINT,
  REST_API_KLINES_ENDPOINT,
  MAIN_SYMBOL,
  REST_API_DAILY_VOLUME
} from '../../consts';
...
export class RestApiService {
  ...
  private loadKlinesAndVolume() {
    ...
    return forkJoin(requests).subscribe((klines: Array<Array<any>>) => {
      this.loadDailyVolume().subscribe((dailyVolume: Array<any>) => {
        const result = this.mainSymbolCoins.map((coin: TradingSymbol, i: number) => {
          // convert from array to WebSocketK
          const convertedKLines = klines[i].map((kline: Array<any>) => convertRestKlineToWebSocketK(kline));
